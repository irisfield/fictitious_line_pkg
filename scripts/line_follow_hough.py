...
def image_callback(camera_image):
    ...
    ###################################################################################################
    lines = cv2.HoughLinesP(filtered_image_with_roi, rho=11, theta=(np.pi / 180),
                            threshold=15, lines=np.array([]), minLineLength=30, maxLineGap=5)

    line_image = np.copy(cv_image)
    for line in lines:
        for x1, y1, x2, y2 in line:
            cv2.line(cv_image, (x1, y1), (x2, int(y2)), (255, 0, 0), 10)
