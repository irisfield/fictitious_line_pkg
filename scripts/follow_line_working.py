...
def image_callback(ros_image):
    ...
    canny = cv2.Canny(gray_image, 200, 300)
    ...
    _, bw_image = cv2.threshold(gray_image, RC.thresh, 255, cv2.THRESH_BINARY)

    #blur_kernel = 1 # must be odd, 1, 3, 5, 7 ...
    #bw_image = cv2.medianBlur(bw_image, blur_kernel)
    ...
    cx -= 170
    drive_to_follow_line_2(cv_image, cx, cy, width, height)
